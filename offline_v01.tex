...
\begin{titlepage}
	...
	{\includegraphics[height = 12mm]{Abb/Logos/Logo_uni_Bild1.png}\par}
	\vspace{1cm}
    ...
    \vfill
   	\includegraphics[height = 20mm]{Abb/Logos/RES_EN_logo_de_rot_mittel.png}
	\hfill	
	\includegraphics[height = 20mm]{Abb/Logos/logo_fb4_Bild2.png}%
	%\vfill
...
Die reversible Zellspannung stellt eine charakteristische Kennzahl der Elektrolyse dar und beschreibt den Punkt, an dem die Zersetzung eintritt. Sie entspricht der Differenz des Kathodenpotentials $E_{Kat}$ und des Anodenpotentials $E_{An}$ (Gleichung(\ref{eq:Teilreakt})). In der Wasserelektrolyse beträgt die Zersetzungsspannung unter Standardbedingungen ($1bar$ und $25 ^ \circ C$) $1,23V$ und wird durch Druck und Temperatur beeinflusst. Nach \cite{Kurzweil.2015} kann, wie in Gleichung(\ref{eq:U_0}) dargestellt, die reversible Zellspannung berechnet werden:
	...
	\end{equation}
			...
			\subsubsection{Kenngrößen der Elektrolyse}
...
Zur Bewertung des Betriebsverhaltens der betrachteten \gls{el}-Technologien in Hinblick auf im vorangestellten Abschnitt(\ref{sect_Betriebsarten}) erläuterte Betriebsarten bzw. -situationen sind verschiedene Kriterien denk- und anwendbar.\\
Um das jeweilige Betriebsverhalten im Einzelnen bewerten und ggf.(?????????????????) eine besondere Eignung einzelner Technologien herausstellen zu können, werden die in Abschnitt(\ref{subsubb_Kenndaten}) erläuterten Kenngrößen 
\begin{itemize}
	\item Menge Produktgas
	\item Systemwirkungsgrad
	\item Spezifischer Energiebedarf
	\item Nutzungsgrad verfügbarer Energie
	\item Wasserstoffgestehungskosten (?????????)
\end{itemize}

genutzt. Deren Abgleich erfolgt, wie nachfolgend erläutert, in unterschiedlicher Kombination für folgende Bewertungskriterien:
\begin{itemize}
	\item Effizienz
	\item Effektivität
	\item Systemdienlichkeit
	\item Ökonomie
\end{itemize} 

Für unterschiedliche Betriebsarten ist dabei ggf. eine verschieden starke Gewichtung einzelner Kriterien vorzunehmen.\\
...
\subsection{Plausibilität des modellierten Verhaltens}
\label{sub_bewkrit_Plausibilitaet}

Um das innerhalb der Simulation gezeigte Betriebsverhalten auf Plausibilität zu prüfen, eignet sich der Abgleich errechneter Kenngrößen mit Literaturwerten. Dazu werden Systemwirkungsgrad \gls{eta_sys} und der spezifische Energiebedarf ????????????? herangezogen.\\  
Zusätzlich sind weitere qualitative Betrachtungen erstellter Zeitreihen und diesbezüglicher Abgleich von Betriebstemperatur \gls{T_stack}, Wertebereichen von Zellspannung \gls{u_cell} und Stromdichte \gls{i} sowie Leistungsbereiche $P_j$ und Produktgasströme einzubeziehen.

\subsection{Effizienz}
\label{subs_bewkrit_effizienz}

Zur Beurteilung der Effizienz einer Anlage werden insbesondere Systemwirkungsgrad \gls{eta_sys} und spezifischer Energiebedarf ????????????? genutzt.\\
Das Kriterium der Effizienz eignet sich vorrangig zum Vergleich der betrachteten Technologien untereinander. Es kann ggf. Aufschluss über eine Eignung der entsprechenden Technologie für spezifische Betriebsarten liefern.

\subsection{Effektivität}
\label{subs_bewkrit_effektivitaet}

Sofern der Einsatz einer \gls{el}-Anlage trotz der hier betrachteten, besonderen Betriebsbedingungen zur Bereitstellung einer bestimmten Menge der Produktgase erfolgt, ist zusätzlich zur Effizienz auch die Effektivität zu betrachten. Diese spiegelt sich maßgeblich in der erzeugten Menge der Produktgase ($m_{H_2}$, $m_{O_2}$)??????????? wider und ist in Bezug zur Anlagenskalierung zu setzen.\\
Auch die Produktgasqualität (??????)spielt in diesem Punkt eine wichtige Rolle.

\subsection{Systemdienlichkeit}%Systemdienstleistung}
\label{subs_bewkrit_systemdienlichkeit}

Nach Definition in Abschnitt(?????????? SystdienlichK) bedeutet Systemdienlichkeit im Kontext dieser Arbeit, einen Beitrag zum Last- und Erzeugungsausgleich des Stromnetzes zu leisten. Hierzu wird im Folgenden vorrangig der Nutzungsgrad $eta_N$ ????????????? herangezogen. Indirekt kann auch die tatsächliche Betriebszeit eines betrachteten Zeitraumes zusätzliche Anhaltspunkte liefern.\\


Das systemdienliche Verhalten einer \gls{el}-technologie kann maßgeblich innerhalb der RESIDUAL-Betriebsart (RES) ???????????? überprüft werden. In Bezug auf einen EISMAN-Betrieb (EIS) soll in diesem Bezug geprüft werden, welche Menge der abgeregelten Energie potentiell zu nutzen ist.\\
Ähnliches gilt für den WEA-Betrieb, wobei hier eine Aussage über die Höhe der nutzbaren Erzeugung und damit indirekt der verbleibenden Einspeisung möglich ist.\\
Bezüglich der betrachteten COST-Betriebsart lässt das während des Betriebs vorliegende Kostenniveau zwar Rückschlüsse auf die entsprechende Netzsituation zu, jedoch ist deren Aussagekraft eingeschränkt (vgl. Abschnitt???????????).\\

\subsection{Ökonomie}
\label{sub_bewkrit_oekonomoie}

Nach den zuvor aufgelisteten, vorrangig technischen Kenngrößen ist für einen sinnvollen Betrieb einer \gls{el}-Anlage ebenfalls die erzielbare Wirtschaftlichkeit von Relevanz.\\
Anhand der spezifischen Wasserstoff-Produktionskosten $k_{H_2}$????????? ist zu überprüfen, od der produzierte Wasserstoff auf dem Markt Konkurrenzfähigkeit besitzt.
...
\begin{figure}[!tbp]
	\centering
	\begin{minipage}[t]{0.49\textwidth}
		\includegraphics[width=\textwidth]{/home/dafu/Schreibtisch/Master-Projekt/Doku/Abb/Modell/inner_NT_23.png}
	...
	\hfill
	\begin{minipage}[t]{0.49\textwidth}
		\includegraphics[width=\textwidth]{Abb/Modell/inner_HT_23.png}
...
\end{wrapfigure} Die Berechnung des optimalen Anlagenstromes erfolgt an Hand einer build-in Optimierungsfunktion.
 Auf Grundlage der Polarisationskurve wird die optimale Stromdichte des Stacks in Abhängigkeit verfügbarer Leistung und unter Berücksichtigung des Peripherie-Leistungsbezugs ermittelt. Die zu Grunde liegende Optimierung wird hier mittels \textsc{Methode kleinster Quadrate} durchgeführt.\\
\newline

Innerhalb der \gls{nt}-Modelle ist eine Nachlaufzeit der Kühlwasserpumpe implementiert. Sinkt die Eingangsleistung auf Werte unterhalb der minimalen Anlagenleistung, 
...
\subsection{Technologiespezifische Parametrierung für Basis-Anlagengröße}
\label{subs_Par_je_TEch}
Für jede Betriebsart wird eine (möglichst optimal) Anlagengröße gewählt

%Keine zeitlichen und infrastrukturellen Ressourcen für Experimente vorhanden.
...
\subsection{Paramter des Temperaturmodells}
\label{sub_params_tempmodell}
Für die Stabilität des thermischen Modells sind Wärmekapazität der Anlage $C_t$, Wärmeübergangskoeffizient $H_{AX} ~=~k \cdot A $?????????????? sowie zu- und abgeführte Wassermengen entscheidend.
Letzteres betreffend werden innerhalb der \gls{nt}-\gls{el} entsprechende Kühlwasserströme mit einem Sicherheitsfaktor versehen, auf den durch Überspannungen maximal hervorgerufenen Wärmeeintrag angepasst und innerhalb der PID-Implementierung geregelt. Die für das Charakteristische Anlagenverhalten in Abhängigkeit einer Temperaturänderung relevanten Parameter $C_t$ und $H_{Ax}$ sind so zu wählen, dass sinnvolle Skalierungseffekte auftreten.\\
Das verwendete Modell zusammengefasster Wärmekapazitäten beinhaltet lediglich einen übergeordneten Wert für die Wärmekapazität eines gesamten Stacks.\cite{glgs} Hieraus entsteht die Schwierigkeit, Zell- und (dem gegenüber) Stack-spezifische Veränderungen der thermischen Systemeigenschaften für unterschiedliche Anlagengrößen zu beziffern.\\
Ein Lösungsansatz besteht darin, aus diversen Literaturwerten einen charakteristischen Zusammenhang von Wärmekapazität und Stack- bzw. Anlagenleistung abzuleiten und mit entsprechendem funktionalen Zusammenhang weiter Werte zu bestimmen. In Ermangelung einer ausreichenden Zahl verschiedener Zahlenwerte kann allerdings dieser Ansatz nicht sinnvoll umgesetzt werden.\\
Aus diesem Grund können für weitere Betrachtungen lediglich entsprechende Zusammenhänge aus Literaturangaben linear extrapoliert werden.???????
Um die Höhe des Fehlers einzugrenzen, werden Werte für Zellanzahl je Stack $N$ und Zellfläche $A_cell$ sowie Wärmekapazität $C_t$ und Wärmeübergangskoeffizient $H_Ax$ nach Möglichkeit aus derselben???? Quelle verwendet.\\
 
Instabiles Verhalten der Temperaturregelung macht es jedoch erforderlich, den Wert des Wärmeübergangskoeffizienten mit Faktor ($2,5$) zu korrigieren.  Mittels überschlägiger Rechnung kann überprüft werden, dass trotz dieser Annahme entsprechende Werte in einem plausiblen bzw. technisch möglichen Bereich liegen.?????????  
% % Espinoza-Lopez beschreibt PSO (S.167 unten links); dennoch sind experimente notwendig....
% % Goeßling -> Tab 1
Auswahl von Fläche, etc. begründen
\subsubsection*{Auswahl Basis-Paramter, PEM}
Für das Basis Modell der \gls{pem}-\gls{el} werden Parameter zu Zellfläche und Zellenzahl je Stack sowie Wärmekapazität und 
...
%	\end{tabular}
%\end{table}
\subsection{Technologiespezifische Skalierung}
...
Zu erkennen ist, dass der Gesamt-Leistungsbezug der Anlage \gls{P_act} über der maximalen Stackleistung liegt. Dies ist insofern sinnvoll, da für diese Betriebsart der maximal Leistungsbezug nicht durch eine verfügbare Leistung, sondern durch die maximale Stackleistung \gls{P_st} begrenzt wird. ?????????????????????????????
...
\section{Ökonomie(???)}
\subsection{Vetriebswege}
Statista: Wasserstoffbedarf 2050
\url{https://de.statista.com/statistik/daten/studie/750972/umfrage/prognose-wasserstoffbedarf-in-deutschland/}
tankstellen:
\url{https://ecomento.de/2018/02/16/wasserstoff-elektroauto-tankstellen-2017-deutschland-europa-welt/}
\url{https://ecomento.de/2019/02/18/deutschland-jetzt-mit-60-wasserstoff-tankstellen/}
...
\begin{table}[]
	...
	\begin{tabular*}{\textwidth}{lllllllll}
		...
		Betriebstemperatur						& $T_{tar}$				& $\degree{C}$ 		& 77 		& \cite{Tjarks.2017}	& 60 					& \cite{Hammoudi.2012}	& 850 					& Ni 		\\[2ex]
		maximale Temperatur						& $T_{max}$				& $\degree{C}$ 		& 85 		& \cite{Tjarks.2017}	& 80 					& \cite{Hammoudi.2012}	& 1050 					& Ni 		\\
		minimale Temperatur						& $T_{min}$				& $\degree{C}$ 		& 20 		& \cite{Tjarks.2017}	& 20 					& \cite{Hammoudi.2012}	& 700 					& Ni 		\\
		maximale Stromdichte 					& $i_{max}$ 			& $A/{cm^2}$ 		& 3.0 		& \cite{Tjarks.2017}	& 0.3 					& \cite{Hammoudi.2012}	& 0.8 					& \cite{Petipas.2013} 	\\
		Zellen pro Stack						& $N_{ce_{st}}$			& -				 	& 200		& \cite{Tjarks.2017}		& 24					& \cite{Hammoudi.2012} 	& 400 					& \cite{Petipas.2013} 	\\
		spez. Wärmekapazität					& $C_{t_{spec}}$		& $kJ/KN_{ce}$ 		& 1.2500	& \cite{Tjarks.2017}		& 4.8906 				& \cite{Ulleberg.2003}		& 0,0969				& \cite{Petipas.2013} 	\\
		spez. Wärmedurchgangskoeffizient		& $H_{AX_{spec}}$		& $W/Kcm^{2}N_{ce}$				& 0.028		& \cite{Gabrielli2016}					& 0,011 	& \cite{Gabrielli2016}		& ??? 		& - 		\\
		spez. Wärmedurchgangswiderstand			& $R_{t_{spec}}$		& $K/W$				& 0,167 	& \cite{Ulleberg.2003}		& 0,167					& \cite{Ulleberg.2003}		& 0,840					& \cite{Petipas.2013} 	\\
		Kühlwassereingangstemperatur			& $T_{cw_{in}}$			& $\degree{C}$ 		& 15 		& \cite{Ulleberg.2003}		& 15 					& \cite{Ulleberg.2003}		& 	 					& 	 		\\	
		Degradationsgradient 					& $deg$ 				& ${\mu}V/h$		& 4-8 		& \cite{Buttler.2018}	& 1-2 					& \cite{Buttler.2018}	& 21 					& \cite{DeNiangThe.12.2015} 	\\
		
		&&&&&&&& \\
		\textbf{Anoden-Kennwerte}&&&&&&&& \\
		Elektrodenmaterial 						& - 					& -					& - 		& -	 		& - 					& \cite{Hammoudi.2012}	& - 					& -			\\
		Elektrodenoberfläche 					& $A_{an}$				& $cm^{2}$			& 170 		& \cite{Gabrielli2016}	& 300					& \cite{Hammoudi.2012} 	& 100 					& \cite{Petipas.2013} 	\\
		Elektrodendurchmesser 					& $\delta_{an}$			& $cm$ 				& 0,14 		& \cite{Chandesris2015} 	& 0,2 					& \cite{Hammoudi.2012}	& 0,05 					& \cite{Zhang.2010} 	\\
		Seperatordistanz 						& $l_{an}$ 				& $cm$ 				& 	 		& 	 		& 0,125 				& \cite{Hammoudi.2012}	& 	 					& 			\\
		
		Tortuosität 							& $\tau_{an}$			& - 				& 	 		& 	 		& 3,65 					& \cite{Abdin.2015}		& 6 					& \cite{Zhang.2010} 	\\
		Porosität 								& $\epsilon_{an}$		& - 				& 	 		& 	 		& 0,30 					& \cite{Abdin.2015}		& 0,3 					& \cite{Zhang.2010} 	\\
		Porenradius (Knudsen-Diff.) 			& $r_{P_{an}}$ 			& $cm$ 				& 	 		& 	 		& $1\times{10^{-4}}$	& \cite{Abdin.2015}		& $0,5\times{10^{-4}}$ 	& \cite{Zhang.2010}  	\\
		Rugosität 								& $rug_{an}$			& $m^{2}/m^{2}$		& 150 		& \cite{Chandesris2015}	& 	 					& 			& 	 					& 	 		\\
		Elektrodenrauheit 						& $\gamma_{an}$ 		& -	 				& 	 		& 	 		& 1,25 					& \cite{Abdin.2015}		&  						& 	 		\\
		
		Ladungstransferkoeffizient 				& $\alpha_{an}$ 		& - 				& 0,65 		& \cite{Chandesris2015}	& 1,65 					& \cite{Abdin.2015}		& 	 					& 	 		\\
		Freie Aktivierungsenergie 				& ${\Delta}G_{c_{an}}$	& $J/mol$ 			& 62836		& \cite{Chandesris2015}	& 80510 				& \cite{Abdin.2015}		& 120000 				& \cite{Zhang.2010} 	\\
		Referenzaustauschstromdichte			& $i_{0ref_{an}}$		& $A/cm^{2}$ 		& 	 		& 	 		& $1\times{10^{-11}}$ 	& \cite{Abdin.2015}		& 	 					& 	 		\\
		Referenztemperatur $i_{0ref_{an}}$ 		& $T_{ref_{an}}$ 		& $\degree{C}$	 	& 	 		& 	 		& 90 					& \cite{Abdin.2015}		& 	 					& 	 		\\	
		Anodic rate parameter					& $k_{0_{an}}$			& $mol/Ksm^{2}$		& $4,63\times{10^{-3}}$ & \cite{Chandesris2015} 				&& 	 		& 	 					& 	 		\\
		Elektrodenleitfähigkeit					& $\sigma_{an}$ 		& $S/m$				& 13700	 	& \cite{Chandesris2015} 	& 						& 	 		& 	 					& 	 		\\
		
		&&&&&&&& \\
		\textbf{Kathoden-Kennwerte}&&&&&&&& \\
		Elektrodenmaterial 						& - 					& - 				& - 		& - 		& -		 				& \cite{Hammoudi.2012} 	& - 					& - 		\\
		Elektrodenoberfläche 					& $A_{ca}$ 				& $cm^{2}$ 			& 170 		& \cite{Gabrielli2016}	& 300 					& \cite{Hammoudi.2012} 	& 100 					& \cite{Petipas.2013}	\\
		Elektrodendurchmesser 					& $\delta_{ca}$ 		& $cm$ 				& 0,235		& \cite{Chandesris2015}	& 0,2 					& \cite{Hammoudi.2012} 	& 0,5 					& \cite{Zhang.2010} 	\\
		Seperatordistanz 						& $l_{ca}$ 				& $cm$ 				& 	 		& 	 		& 0,125 				& \cite{Hammoudi.2012} 	& 	 					& 	 		\\
		
		Tortuosität 							& $\tau_{ca}$	 		& -	 				& 	 		& 			& 3,65 					& \cite{Abdin.2015} 	& 6 					& \cite{Zhang.2010}  	\\
		Porosität 								& $\epsilon_{ca}$		& -	 				& 	 		& 			& 0,30 					& \cite{Abdin.2015} 	& 0,3 					& \cite{Zhang.2010}  	\\
		Porenradius (Knudsen-Diff.) 			& $r_{P_{ca}}$			& $cm$ 				& 	 		& 	 		& $1\times{10^{-4}}$	& \cite{Abdin.2015} 	& $0,5\times{10^{-4}}$ 	& \cite{Zhang.2010}  	\\
		Rugosität 								& $rug_{ca}$			& $m^{2}/m^{2}$		& 150 		& \cite{Chandesris2015}	& 	 					& 		 	& 	 					&	 		\\
		Elektrodenrauheit 						& $\gamma_{ca}$ 		& 	 				& 	 		& 	 		& 1,05 					& \cite{Abdin.2015} 	& 	 					& 	 		\\
		
		Ladungstransferkoeffizient 				& $\alpha_{ca}$ 		& - 				& 0,51 		& \cite{Chandesris2015}	& 0,73 					& \cite{Abdin.2015} 	& 	 					& 	 		\\
		Freie Aktivierungsenergie 				& ${\Delta}G_{c_{ca}}$ 	& $J/mol$		    & 24359		& \cite{Chandesris2015}	& 80510 				& \cite{Abdin.2015} 	& 100000 				& \cite{Zhang.2010} 	\\
		Referenzaustauschstromdichte			& $i_{0ref_{ca}}$ 		& $A/cm^{2}$		& 	 		& 	 		& $1\times{10^{-3}}$ 	& \cite{Abdin.2015} 	& 	 					& 	 		\\
		Referenztemperatur $i_{0ref_{ca}}$ 		& $T_{ref_{ca}}$ 		& $\degree{C}$	 	& 	 		& 	 		& 90 					& \cite{Abdin.2015} 	& 	 					& 	 		\\
		Cathodic rate parameter					& $k_{0_{ca}}$			& $mol/Ksm^{2}$		& $1,10\times{10^{-3}}$ & \cite{Chandesris2015} 				&&			& 						& 	 		\\
		Elektrodenleitfähigkeit					& $\sigma_{ca}$ 		& $S/m$				& 46	 	& \cite{Chandesris2015}	& 	 					&  			& 	 					& 	 		\\
		
		&&&&&&&& \\
		\textbf{Diaphragma-Kennwerte}&&&&&&&& \\
		Oberfläche 								& $A_{dia}$ 			& $cm^{2}$ 			&&& 300 	& \cite{Abdin.2015} 		&&\\
		Durchmesser 							& $\delta_{dia}$ 		& $\mu{m}$ 			&&& 320 	& \cite{Abdin.2015} 		&&\\
		Tortuosität 							& $\tau{dia}$			& - 				&&& 3,65 	& \cite{Abdin.2015} 		&&\\
		Porosität 								& $\epsilon_{dia}$ 		& - 				&&& 0,30 	& \cite{Abdin.2015} 		&&\\
		Wettability 							& $\omega_{dia}$ 		& - 				&&& 0,85 	& \cite{Abdin.2015} 		&&\\
		Spezifische Resistivität 				& $\rho_{dia}$ 			& ${\Omega}/cm$ 	&&& 1,5 	& Perl ??? Zirf &&\\
		
		&&&&&&&& \\
		\textbf{Elektrolyt-Kennwerte}&&&&&&&& \\
		Durchmesser								& $\delta_{mem}$		& $\mu{m}$			& 200		& \cite{Gabrielli2016}	&			&			& 500   & \cite{Zhang.2010} \\			
		Membranfeuchtigkeit						& $\lambda_{mem}$		& -					& 16  		& \cite{Chandesris2015}	& 			& 			& 		& 		\\
		Membranwiderstand						& $R_{mem}$ 			& $\Omega$			& 0.096		& \cite{Tjarks.2017}	& 	 		& 			& 	 	& 		\\
		Diff. Permeabilität $O_{2}$				& $\epsilon_{O_{2}}$	& $mol/cmsbar$		& $2,00\times{10^{-11}}$	& \cite{Schalenbach2013}			&			&						&	 \\
		Diff. Permeabilität $H_{2}$				& $\epsilon_{H_{2}}$	& $mol/cmsbar$		& $4,65\times{10^{-11}}$	& \cite{Schalenbach2013}			&			&						&	 \\
		
		&&&&&&&& \\
		KOH-Konzentration 						& - 					& $wt\%$ 			&			&			& 30 		& \cite{Henao.2014}		&		& 		\\
		KOH-Volumenstrom pro Stack 				& $V_{KOH}$ 			& $l/min$ 			&			&			& 0.5 		& \cite{Henao.2014}		&		& 		\\
